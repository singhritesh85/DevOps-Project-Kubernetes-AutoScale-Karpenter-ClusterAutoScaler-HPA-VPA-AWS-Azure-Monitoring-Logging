...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  ...
  default_node_pool {
    name                 = "agentpool"
    vm_size              = "Standard_DS3_v2"      ###Standard_B2s     ###Standard_DS3_v2      ###Standard_DS2_v2      ###Standard_B2ms   
...
resource "azurerm_monitor_metric_alert" "alert_rule2" {
  ...
  resource_group_name = azurerm_resource_group.aks_rg.name
  scopes              = [azurerm_kubernetes_cluster.aks_cluster.id]
  description         = "Alert when Memory Utilization is greater than 80%"
...
resource "azurerm_monitor_metric_alert" "alert_rule3" {
  name                = "${var.prefix}-alert-rule3"
  resource_group_name = azurerm_resource_group.aks_rg.name
  scopes              = [azurerm_kubernetes_cluster.aks_cluster.id]
  description         = "Alert when Disk Utilization is greater than 80%"
  auto_mitigate       = true    ### Metric Alert to be auto resolved
  frequency           = "PT5M"
